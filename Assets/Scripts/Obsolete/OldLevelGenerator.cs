...
public class OldLevelGenerator : MonoBehaviour
{
    ...
    {
        GameObject room = Instantiate(Room, new Vector3(x * OLDWallModel.INNER_RADIUS + (OLDWallModel.INNER_RADIUS * y / 2f), 0f, ((OLDWallModel.OUTER_RADIUS + OLDWallModel.WALLSIZE) / 2f)*y), Quaternion.identity, transform);
        room.name = $"Room {{{x},{y}}}";

        OLDWallModel walls = room.GetComponent<OLDWallModel>();
        walls.Index = new Vector2Int(x, y);

        WallType[] types = new WallType[OLDWallModel.NUMBER_OF_WALLS];
